...
import { DeleteUserByUid, GetPeopleDirectoryList, PostUnassignedUsers } from '../helpers/Api';
...
interface PeopleDirectoryTable {
    handleUnassignedClick: (selectedUserUid: string) => void;
    handleDeleteClick: (selectedUserUid: string) => void;
}
...
export function PeopleDirectoryTable(props: PeopleDirectoryTable) {
    const { user, business, session } = useAuth();
    const [search, setSearch] = useState('');
    const [sortedData, setSortedData] = useState<RowData[] | null>(null);
    const [peopleData, setPeopleData] = useState<RowData[] | null>(null);
    ...
    const handleUnassignedClickProp = props.handleUnassignedClick;

    async function fetchData() {
        if (user && business) {
            // get 
            var params: ParamFields = {
            ...
            //setLoading(true);
            var peopleDirectoryData = await GetPeopleDirectoryList(params, session?.access_token);
            if (peopleDirectoryData !== undefined) {
                var fmlist = peopleDirectoryData['staffs'];
                setPeopleData(peopleDirectoryData['staffs']);
                setLoading(false);
            }
        }
    }
    ...
    const rows = peopleData?.map((row: RowData) => (
        <>
            {isMobile && (
    ...
    const sortedRows = sortedData?.map((row: RowData) => (
        <>
            {isMobile && (
                <Paper shadow="md" p="lg" radius="lg" style={{ background: "#354E40", color: "#dcdcdc" }} mt="sm">
                    <RowItemMobile name={row.name} uid={row.uid} email={row.email} position={row.position}/>
        ...
        <>
            ...
            {deleteUserModalOpened && (
                <DeleteUserConfirmModal
                    ...
                    handleDeleteClick={() => handleDeleteClickProp(selectedUserUid)}
                    handleUnassignedClick={() => handleUnassignedClickProp(selectedUserUid)}
                />
            )}
                ...
                <Stack>
                    <ScrollArea>
                        ...
                        {!loading && (sortedRows && sortedRows.length > 0 || rows && rows.length > 0) && (
                            <Paper shadow="md" p="md" radius="lg" style={{ background: "#649181", color: "#dcdcdc" }} mt="md">
                                {isMobile && (
                                    <Grid>
                                        <Grid.Col span={{ base: 8 }}>
                                            <Text size="20px" style={{ fontFamily: "AK-Medium" }}>Name</Text>
